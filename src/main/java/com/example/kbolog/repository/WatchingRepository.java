...
// dashboard - 사용자 관람 기록(로그) ✅
    ...
    List<Watching> findByUser(@Param("userId") Long userId);

// log - 중복 로그 있나 확인 ✅
    @Query(value = "SELECT EXISTS (SELECT 1 FROM watching WHERE game_id = :gameId AND user_id = :userId)",
            nativeQuery = true)
    Integer existsByGameAndUser(@Param("gameId") Long gameId, @Param("userId") Long userId);



    @Query(value = "SELECT * FROM watching WHERE watching_id = :id", nativeQuery = true)
    Optional<Watching> findByWatchingId(@Param("id") Long id);
...
//    log - 로그 작성 ✅
    ...
    void write(@Param("userId") Long userId,
